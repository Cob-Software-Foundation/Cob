# Cob Toolchain Installer - Hardcoded Route Matching
...
if ($RawArch -eq "AMD64" -or $FallbackArch -eq "AMD64") {
...
elif ($RawArch -eq "ARM64" -or $FallbackArch -eq "ARM64") {
    ...
    # Fallback default if an unusual architecture reports back
...
if (Test-Path $InstallDir) {
    Write-Host "🧹 Cleansing old installation directory at $InstallDir..." -ForegroundColor Yellow
    Remove-Item $InstallDir -Recurse -Force
}
New-Item -ItemType Directory -Force -Path $InstallDir | Out-Null

# 4. Fetch the Target Payload
Write-Host "📡 Fetching artifact from: $DownloadUrl" -ForegroundColor Cyan
...
Write-Host "💡 Restart your PowerShell terminal session and run 'cob_interp' to check your version." -ForegroundColor Magic
